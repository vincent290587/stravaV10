      ...
      switch (state->decode_state)
      {
         case FIT_CONVERT_DECODE_FILE_HDR:
            if (state->mesg_offset < FIT_FILE_HDR_SIZE) {
               state->u.mesg[state->mesg_offset] = datum;
               //memcpy((FIT_UINT8 *) &state->u.file_hdr + state->mesg_offset, &datum, sizeof(FIT_UINT8));
//               printf("datum %c\r\n", (char)datum);
            }
...
//               printf("%02X %02X %02X %02X  offset %u %d\r\n",
//            		   state->u.file_hdr.data_type[0],
//					   state->u.file_hdr.data_type[1],
//					   state->u.file_hdr.data_type[2],
//					   state->u.file_hdr.data_type[3],
//					   state->mesg_offset,
//					   state->u.mesg + 8 == (FIT_UINT8 *) &state->u.file_hdr.data_type
//               );
//               printf("%lu %lu %lu\n", state->u.mesg, (FIT_UINT8 *) &state->u.file_hdr, sizeof(uint32_t));
//               printf("size: %lu \n", sizeof(state->u.file_hdr));
//               printf("%lu %lu \n", state->u.mesg + 4, (FIT_UINT8 *) &state->u.file_hdr.data_size);
